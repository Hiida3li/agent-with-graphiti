...
"""
    ...
    def _search_products(self, text: str, image: bool = False, image_url: List[str] = None,
        ...
        end_time = time.time()
        logger.info(
            f"[Thread {thread_id}] FINISHED search_products at {end_time:.3f} (took {end_time - start_time:.3f}s)")
    ...
    def _search_faqs(self, text: str) -> str:
        ...
        thread_id = threading.get_ident()
        start_time = time.time()
        logger.info(f"[Thread {thread_id}] STARTED search_faqs at {start_time:.3f}")
        ...
        if "shipping" in text.lower():
            result = "Shipping Policy: We offer free standard shipping on orders over $50. Express shipping available for $9.99. Orders typically ship within 1-2 business days."
        elif "return" in text.lower():
            result = "Return Policy: 30-day return window for unused items in original packaging. Free returns on defective items."
        else:
            result = f"FAQ information related to: {text}"

        end_time = time.time()
        logger.info(f"[Thread {thread_id}] FINISHED search_faqs at {end_time:.3f} (took {end_time - start_time:.3f}s)")
        return result
...
def main():
    """Interactive compositional agent"""
    ...
    print("Interactive Compositional Agent")
    ...
    while True:
        ...
        try:
            result = agent.execute_composition(user_query, image_urls)
