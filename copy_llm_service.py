...
def cleanup_context(context: Dict[str, Any], max_interactions: int = 5, max_history: int = 20):
    """Keep only recent interactions and history"""

    # Keep only last N interactions
    if "interactions" in context and len(context["interactions"]) > max_interactions:
        context["interactions"] = context["interactions"][-max_interactions:]
        logger.info(f"Cleaned up interactions, keeping last {max_interactions}")

    # Keep only last N history entries
    if "history" in context and len(context["history"]) > max_history:
        context["history"] = context["history"][-max_history:]
        logger.info(f"Cleaned up history, keeping last {max_history}")

    return context


def check_context_size(context: Dict[str, Any]) -> Dict[str, Any]:
    """Monitor context size and trigger cleanup if needed"""
    context_json = json.dumps(context, default=str)
    size_kb = len(context_json.encode('utf-8')) / 1024

    logger.debug(f"Context size: {size_kb:.1f}KB")

    if size_kb > 100:  # Alert if over 100KB
        logger.warning(f"Context size: {size_kb:.1f}KB - triggering cleanup")
        return cleanup_context(context, max_interactions=3, max_history=10)
...
def process_message(msg: Dict[str, Any], llm_provider: LLMProvider) -> List[Dict[str, Any]]:
    ...
    # Emergency cleanup to prevent failures
    if len(json.dumps(context, default=str)) > 50000:  # 50KB limit
        logger.warning("Emergency context cleanup triggered - context too large")
        context = {
            "query": context.get("query", ""),
            "interactions": context.get("interactions", [])[-2:],  # Keep only last 2
            "history": context.get("history", [])[-5:]  # Keep only last 5
        }
        logger.info("Emergency context cleanup performed")

    # Clean up memory before processing
    context = cleanup_context(context)

    # Monitor context size
    context = check_context_size(context)
    ...
    if function_calls:
        ...
        logger.error("No function calls generated - this should not happen")
        # Fallback: create emergency format_response call
        emergency_func_call = {
            "name": "final_response",
            "args": {
                "content": "I'm experiencing technical issues. Please try again.",
        ...
        function_execution = {
            "execution_id": emergency_func_call["id"],
            "function_name": emergency_func_call["name"],
            "parameters": emergency_func_call["args"],
        ...
        message = {
            ...
            "payload": {
                "agent": {
                    "context": context,
                    "prompt": prompt_template,
                    "current_function_execution": emergency_func_call,
...
def process_function_response(msg: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if "response" in function_call:
        ...
        # Don't store full results in history - make it lightweight
        tool_response_log = f"Tool Response: Success ({function_name})"
        ...
        for interaction in interactions:
            for execution in interaction.get("function_executions", []):
                if execution.get("execution_id") == func_call_id:
                    ...
                    # Compress function results - store only essential data
                    if function_name in ["search_products", "search_faqs"]:
                        execution["execution_result"] = {
                            "success": True,
                            "summary": function_result.get("formatted_summary", "")[:500],  # limit size
                            "product_count": len(
                                function_result.get("products", [])) if "products" in function_result else 0
                        }
                    else:
        ...
        if latest_interaction:
            ...
            if all_completed:
                ...
                # Check if the last execution was final_response
                last_execution = executions[-1] if executions else None
                if last_execution and last_execution.get("function_name") == "final_response":
                    ...
                    # Need to call final_response - create a summary of function results
                    logger.info("Creating summary for final_response call")

                    # Build content from function results
                    content_parts = []
                    user_query = context.get("query", "")
                    content_parts.append(f"User asked: {user_query}")

                    for execution in executions:
                        if execution.get("execution_status") == "completed" and execution.get(
                            ...
                            if "formatted_summary" in result:
                                content_parts.append(f"Found: {result['formatted_summary']}")
                            elif "success" in result and result["success"]:
                                content_parts.append(f"Function {func_name} completed successfully")

                    summary_content = "\n\n".join(
                        content_parts) if content_parts else "I'm here to help you with your inquiry."
